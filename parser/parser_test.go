...
func TestIntegerLiteralExpression(t *testing.T) {
	...
	if literal.Value != 5 {
		t.Errorf("literal.Value not %d. got=%T", 5, stmt.Expression)
	}
	if literal.TokenLiteral() != "5" {
		t.Errorf("literal.TokenLiteral() not %d. got=%T", 5, literal.TokenLiteral())
	}
}
...
func TestParsingPrefixExpressions(t *testing.T) {
	...
	for _, tt := range prefixTests {
		...
		if exp.Operator != tt.operator {
			t.Fatalf("literal.Value not %d. got=%T", 5, stmt.Expression)
		}
